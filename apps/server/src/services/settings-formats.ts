...
const LAUNCH_CONF_HEADER = [
  ...
  '# Sourced by start.sh; values are shell-quoted and charset-restricted.',
]

// `key value` pairs, one per line, plus the EXTRA_ARGS line start.sh
// actually consumes. The pair lines are the panel's own record of what
// it rendered — parsing reads them back, not EXTRA_ARGS.
export const launchConfFormat: SettingsFormat = {
  parse(content) {
    const doc = docFrom([])
    for (const text of content.split('\n')) {
      const trimmed = text.trim()
      if (!trimmed || trimmed.startsWith('#') || trimmed.startsWith('EXTRA_ARGS=')) continue
  ...
  serialize(doc) {
    ...
    // Single quotes: no expansion, and the charset check already ruled
    // out a quote character that could close the string early.
    lines.push(`EXTRA_ARGS='${args.join(' ')}'`)
    return `${lines.join('\n')}\n`
  },
